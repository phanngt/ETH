...
def swap_uniswap_eth_to_usdc(account: LocalAccount):
    ...
    hex_data = ""
    ...
    # tuples = (Si(BaseWeb3).uniswapv3.token0.lower(), Si(BaseWeb3).uniswapv3.token1.lower(), 10000, -887200, 887200, 19999999999999999943871, 12560042229378440, 19950186722152658004467, 2560042229378440, sender_address.lower(), int((datetime.datetime.utcnow() + datetime.timedelta(minutes=5)).timestamp()))
    tuples = (Si(BaseWeb3).uniswapv3.token0.lower(), Si(BaseWeb3).uniswapv3.token1.lower(), 500, sender_address.lower(), 10000000000000000, 5821154785582746276349, 0)
    ...
    gas = contract.functions.multicall(1680257528, [byte_data]).estimate_gas({'from': sender_address})
